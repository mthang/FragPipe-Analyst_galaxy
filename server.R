...
########### Newly added for Galaxy Interactive tools ####################
  # Check input path

  file_path <- reactive({
    if (dir.exists("external_data")) {
      # use all .txt and .csv files specified in external_data
      external <- sapply(list.files(path = "external_data", 
                                    pattern = "\\.txt$|\\.tsv$|\\.csv$|\\.xls$|\\.xlsx$",
                                    recursive = T), 
                         function(x){ file.path("external_data", x)})
      print(external)
      return(external)
    } else {
      return(NULL)
...
########## Newly added section for Galaxy Interactive tool ##############

  observe(
    if(!is.null(file_path())){
      shinyjs::hide("upload_area")
    }
  )
...
########## Added/modified section for Galaxy Interactive Tools #######################
       if (input$exp == "LFQ"){
	 ## check missing files
         #inFile <- input$lfq_expr
         #exp_design_file <- input$lfq_manifest
	  if (is.null(fragpipe_data_input()) | is.null(exp_design_input())) {
	      shinyalert("Input file missing!", "Please check your input files", type="warning",
        	...
        	return(FALSE)
              ...
              shinyalert("Incorrect input file!", "The replicate number cannot duplicate under same condition, please check your input files", type="warning",
                   ...
                   timer = 0)
             return(FALSE)
      	   } else if (any(!exp_design_input()$label %in% colnames(fragpipe_data_input()))){
              shinyalert("Incorrect input file!", "Not all labels in experimental design file match with protein expression values", type="warning",
                   closeOnClickOutside = FALSE,
                   closeOnEsc = TRUE,
                   timer = 0)
              return(FALSE)
...
######### Modified for Galaxy Interactive Tools ###########################
	if(!is.null(file_path())){
	    inFile <- file_path()[grep("^combined_protein", names(file_path()),ignore.case = T)]
         } else {
	   ...
	   temp_data <- read_excel(inFile)
  	 } else {
	   ### replace read.table by read.csv
	   ### error occur
	   temp_data <- read.csv(inFile,
				   header = TRUE,
				   fill = TRUE,
				   sep= "\t",
				   quote = "",
				   ...
				   check.names = FALSE)
	  # comment out section
	  # no such columns in the input tsv files
	  # intensity_columns<- grep("Abundances \\(Grouped\\):", colnames(temp_data))
    	  # colnames(temp_data)[intensity_columns] <- colnames(temp_data)[intensity_columns] %>% gsub(", Sample", "",.)
	   return(temp_data)
	 }
######### End Modification section ######################################
      ...
      if(is.null(inFile))
        return(NULL)
      temp_data <- read.csv(inFile$datapath,
                 header = TRUE,
                 fill= TRUE, # to fill any missing data
                 sep = "\t",
                 quote = "",
...
############## add by Mike  ###############
  ...
  exp_data1 <- reactive({
     print("hello world")
    #req(input$lfq_expr)
    #print(input$lfq_expr)
    if (!is.null(input$lfq_manifest)){
      tempTable <- read_exp_func(input$lfq_manifest) 
      print(tempTable)
      # change column names to lowercase; batch to upper
      colnames(tempTable) <- tolower(colnames(tempTable))
      tempTable$batch <- tempTable$batch %>% toupper()
      # correct the replicate to integer
      tempTable$replicate = as.integer(tempTable$replicate)
    } else {
      if (endsWith(input$lfq_expr$datapath,"xlsx")){
        df<- read_excel(input$lfq_expr$datapath)
      } else {
        df <- read.table(input$lfq_expr$datapath,
                                header = TRUE,
                                fill= TRUE, # to fill any missing data
                                sep = "\t",
                                comment.char = "",
      ...
      tempTable =  get_tmt_exp_design(df)
    }
    print(tempTable)
    ...
    tempTable$label <- ifelse(grepl("^[[:digit:]]", tempTable$label), paste("X",tempTable$label,sep = ''), tempTable$label) # TODO: check if this will affect the testing
    tempTable$label <- tempTable$label %>% gsub(", Sample", "",.) #TODO: find a better way to avoid this 
...
############# updated section ####################################################
######## replaced read.delim by read.table and inFile$datapath by inFile
	...
	temp_df <- read.table(inFile,
	  ...
	  print("before label column")
          temp_df$label <- temp_df$sample
	  print("after label column")
